...
def _get_secret_key():
    result = get_config_value_as_str(CONFIG_GUI, CONFIG_GUI_SECRET_KEY)
    if result:
        return result
...
class Config:
    def __init__(self):
        ...
        self.INSTANCE_NAME = get_config_value_as_str(CONFIG_GLOBAL, CONFIG_GLOBAL_INSTANCE_NAME)
        ...
        if get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_UNIX_SOCKET):
            self.SQLALCHEMY_DATABASE_URI = 'mysql+pymysql://{username}:{password}@localhost/{database}?unix_socket={unix_socket}&charset=utf8mb4'.format(
                username=get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_USERNAME),
                password=get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_PASSWORD),
                unix_socket=get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_UNIX_SOCKET),
                database=get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_DATABASE))
        else:
            self.SQLALCHEMY_DATABASE_URI = 'mysql+pymysql://{username}:{password}@{hostname}:{port}/{database}?charset=utf8mb4'.format(
                username=get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_USERNAME),
                password=get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_PASSWORD),
                hostname=get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_HOSTNAME),
        ...
        if get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_MAX_ALLOWED_PACKET):
        ...
        if not get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_UNIX_SOCKET):
            if get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_CA) \
            or get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_CERT) \
            or get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_KEY):
                ssl_options = { 'ca': abs_path(get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_CA)) }
                if get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_CERT):
                    ssl_options['cert'] = abs_path(get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_CERT))
                if get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_KEY):
                    ssl_options['key'] = abs_path(get_config_value_as_str(CONFIG_DATABASE_ACE, CONFIG_DATABASE_SSL_KEY))
