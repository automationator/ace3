...
class BrotexHTTPPackageAnalysis(Analysis):
    ...
    @requests.setter
    def requests(self, value: list[dict]):
        self.details[KEY_REQUESTS] = value
...
class BrotexHTTPPackageAnalyzer(AnalysisModule):
    ...
    def execute_analysis(self, _file) -> AnalysisExecutionResult:
        ...
        if not _pattern_brotex_package.match(_file.file_path):
            logging.debug("{} does not appear to be a brotex http package".format(_file))
            return AnalysisExecutionResult.COMPLETED

        analysis = self.create_analysis(_file)
        assert isinstance(analysis, BrotexHTTPPackageAnalysis)
        ...
        for message_number in message_dirs.keys():
            ...
            if http_request[KEY_HOST]:
                analysis.add_observable_by_spec(F_FQDN, http_request[KEY_HOST])

            if http_request[KEY_URI]:
                analysis.add_observable_by_spec(F_URL, http_request[KEY_URI])
