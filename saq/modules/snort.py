...
class SnortSignatureAnalysis_v1(Analysis):
    ...
    def signature(self) -> Optional[str]:
        return self.details[KEY_SIGNATURE] if self.details else None
...
class SnortSignatureAnalyzer_v1(AnalysisModule):
    ...
    def execute_analysis(self, snort_sig) -> AnalysisExecutionResult:
        ...
        self.get_root().add_summary_detail(header=f"Suricata Signature {analysis.signature_id}", content=analysis.signature, format=SUMMARY_DETAIL_FORMAT_TXT)
        return AnalysisExecutionResult.COMPLETED
